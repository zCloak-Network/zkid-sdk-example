...
(async () => {
    ...
    // Step 7: get message from the first attester
    const serverMsgOrigin2 = await getMessage(
        attester,
        attester2,
        "Extends_Request_Comfirmation"
    );

    // Step 8: decrypt request comfirmation message
    // serverMsg array is just for demo use, the details should be based on the actual situation
    const decrypted2 = await decryptMessage(serverMsgOrigin2[0], attester2);

    // Step 9: get VerifiableCredential from decrypt return value
